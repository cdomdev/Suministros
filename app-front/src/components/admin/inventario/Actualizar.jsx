...
export const Actualizar = ({ productId, productInfo }) => {
  ...
  const handleInputChange = (e) => {
    ...
    const productosActulizado = {
      ...
      categoria_Id: selectedCategoryId || productInfo.categoria_id,
    };
    ...
    if (productosActulizado) {
      axios
        ...
        .catch((error) => {
          console.log(error)
          if(response.status === 400 || respose.status === 500){
  ...
  return (
    <>
      <Button variant="outline-success" className="btn-custome-inventary" onClick={() => setShowModal(true)}>
